...
public class Board {
    ...
    static ArrayList<Chancekort> kort = new ArrayList<>();
    ...
    * */
    ...
    //Text, expense, income
    private void createChanceCards(String[] chanceData) {
        for (int i = 0; i < fields.length; i++) {
            String s = chanceData[i];
            String[] values = s.split(", ");

            String text = (values[0]);
            int expense = Integer.parseInt(values[1]);
            int income = Integer.parseInt(values[2]);

            Chancekort c = new Chancekort(text, expense, income);
            kort.add(c);
        }
    }

        public static Chancekort getChanceCard(){
            Chancekort currentCard = kort.get(index);
            index++;
            if(index > kort.size()){
                index = 0;
            }
